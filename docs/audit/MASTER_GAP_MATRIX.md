...
| G7 | §13 | Trust Identity duplicate-handling + account-recovery depth (link-identity-to-existing-account flows) | P2 | **CLOSED (Batch 2)** — fail-closed IDENTITY_TAKEN guard at the verification callback (session FAILED, IDENTITY_DUPLICATE_BLOCKED audit, SECURITY notification to the holding account, no holder disclosure); frontend recovery panel routes to the existing account / Forgot-password / support; revoked/expired claims release the subject | 2 | CURRENT_STATE §4 |
| G8 | §53 | Business/RC-number identity model absent | P2 | **CLOSED (Batch 2 — model + UI only, per DB1)** — BusinessAccount schema (peppered rcFingerprint + masked hint, raw RC never stored), owner-scoped CRUD + honest UNVERIFIED-only status; cross-account duplicates allowed (no provider = no adjudication); no-trust-signal invariant enforced by tests/batch2_matrix.py | 2 | DATABASE_AUDIT DB1 (UNCONFIRMED provider flow — model only) |
...
## Batch-2 gate decision

Deliverables: G7 CLOSED (duplicate government identity refused fail-closed at the verification callback — 409 IDENTITY_TAKEN, session FAILED `identity_taken`, IDENTITY_DUPLICATE_BLOCKED audit, SECURITY notification to the HOLDING account, zero holder disclosure; frontend recovery panel routes the legitimate owner to the existing account / password recovery / support; a REVOKED holder claim releases the subject), G8 CLOSED as scoped (BusinessAccount model + owner CRUD + passport UI slot, RC/BN/IT normalization, peppered fingerprint + masked hint — raw RC never stored; UNVERIFIED-only status, cross-account duplicates allowed because no provider means no adjudication; the no-trust-signal invariant — no score/band/level/trust keys, no raw RC digits in any businesses response — is matrix-enforced), PR2 CLOSED (tests/batch2_matrix.py §C: DMMF-driven erasure-cascade invariant — after DSR DELETE, EVERY model with a UserAccount relation (any FK name) must hold 0 rows for the user; the same check asserts every UserAccount relation in the schema is onDelete: Cascade, so future models cannot silently escape; AuditEvent tombstones survive by design; ada is the untouched surgical-cascade negative control). ALSO FIXED en route: a live API-catalog drift (19 on-disk routes undocumented in the /api index — 14 AUTH-batch + admin/security-posture + the 4 new business routes; batch1's drift guard now walks a closed 107-entry catalog). Regression: batch2_matrix 59/59 ×2 (idempotent), stage2 36/36 (after the documented shared-IP rate-window pause), batch1 28/28, stage9 102/102, batch0 29/29, auth_matrix 95/95, tsc 0 errors, ESLint clean, agent-browser E2E verified (business card create/rename/remove; identity-taken panel render/dismiss; 390px no-overflow; console clean). **Batch 2 PASSES its gate.**
