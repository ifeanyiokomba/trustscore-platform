...
| --- | --- | --- | --- | --- | --- | --- |
| G1 | §42/§43 | No CSP/security headers in `next.config.ts` | P1 | **OPEN — Batch 0 fix** | 0 | SECURITY_AUDIT S1 |
| G2 | §22 | `/v1/trust/check` lacks `purpose` (transaction-specific trust); receipts not purpose-labeled | P1 | **OPEN — Batch 0 fix** | 0 | PRODUCT_AUDIT P1, API_AUDIT AP1, DATABASE_AUDIT DB3 |
| G3 | §37/UX | NINAuth brand-compliance CI assertion (label + ≥44px target) missing | P1 | **OPEN — Batch 0 fix** (test) | 0 | UX_AUDIT UX3 |
| G4 | §8/§43 | LIVE NINAuth contract unknowns (20 UNCONFIRMED rows: base URL, token auth, scopes, errors, webhooks, nonce semantics) | P2 | **BLOCKED** on partner credentials | 1 (checklist ready) | NINAUTH_CONTRACT_MATRIX |
| G5 | §11 | Capability→production-scope mapping not externalized as config | P2 | SCHEDULED | 1 | PROVIDER_AUDIT PV2 |
| G6 | §2.2/§6 | Tripwire depth-bound regression test for new claim shapes | P3 | SCHEDULED | 1 | ARCHITECTURE_AUDIT A2 |
...
| G18 | §15/§16 | Purpose↔requestReason adapter mapping (39-key catalog) for LIVE | P2 | **BLOCKED** on partner | 1 | PRIVACY_AUDIT PR1 |
