...
## Follow-up — external review, round 2 (2026-09-19, commit ba88f05)

Re-verified the round-1 disposition table above against the actual current code (not the table's own claims) after the sec-batch-A push. F1, F2, F3, F4, F6, F7, F8, F10 all checked out — confirmed by direct read plus, where practical, by actually running the thing (`npm audit` for F3, a fresh `tsc --noEmit` for F1/F5, a Next.js file-conventions check for F2's `proxy.ts` naming, a truth-table check for the new gate below). F9/F11/F12 unchanged, as expected — out of scope for this batch.

**One discrepancy worth recording plainly:** the F5 row above states tsc was verified at 0 errors before the `ignoreBuildErrors` flip. Running `tsc --noEmit` here still shows the same 3 errors from round 1 (`devportal-service.ts:560`, `network-service.ts:716`, `score-insights-service.ts:309/310`), plus one new one in `tests/batch2_db_ops.ts:68`. Likely explanation, not a contradiction: this sandbox can't reach `binaries.prisma.sh`, so `prisma generate` fails here and the check is running against a stale client from before the schema grew (`AuthIdentifier`, `GoogleLoginSession`, business tables). Recorded rather than silently dropped — worth a real `bunx tsc --noEmit` wherever Prisma actually resolves, to close this out with certainty either way.

**New finding — Google Sign-In's LIVE path was structurally half-built.** `googleAuthorizationUrl()` builds a correct real-Google redirect for LIVE mode; `completeGoogleLogin()` unconditionally minted a MOCK-signed token regardless of mode (no real token-endpoint exchange, no RS256/JWKS verification against a genuine Google ID token). Not reachable today — `GOOGLE_CLIENT_ID`/`GOOGLE_CLIENT_SECRET` aren't in `.env.example` — but the LIVE branch looking finished is exactly the setup for someone flipping it later and hitting an undefined failure (most likely a crash on `session.googleSub` being unset, since only the MOCK-only `/grant` endpoint populates it) in front of a real user.

Fixed this round rather than building an unverifiable real OAuth implementation: this sandbox has no path to `googleapis.com` either, so writing the real token-exchange/JWKS code here would have been unverifiable by construction — worse for the system than being honest about what isn't done yet. Added `assertGoogleLiveNotAttempted()` in `google.ts`, following the exact pattern already proven for `PROVIDER_LIVE_ENABLED`: throws in production (or `TS_BOOT_GUARD_STRICT=1`) the instant `GOOGLE_MODE` would resolve to `"LIVE"`, dev stays unguarded so the real implementation can still be built locally. Wired at both the boot check (`src/proxy.ts`, alongside `assertSecretsAtBoot()`) and, defense-in-depth, inside `startGoogleLogin()` and `completeGoogleLogin()` directly. Verified: `tsc --noEmit` clean on all 3 touched files (`google.ts`, `proxy.ts`, `google-auth-service.ts`) with no new errors introduced, plus a standalone truth-table check of the guard's 6 mode/env/strict combinations, all behaving as designed. Remove the guard once real Google token verification ships.
