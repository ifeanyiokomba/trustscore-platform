...
check("health reports the current platform version (Stage 15 / v1.14.0)",
      s == 200 and b.get("version") == "1.14.0" and "15" in b.get("stage", ""),
      str(b.get("stage")))
s, b = call(client(), "GET", "/api")
paths = {e["path"] for e in b.get("endpoints", [])}
check("api index: v1.14.0 + Stage 15 (provider endpoints still cataloged)",
      b.get("version") == "1.14.0" and "15" in b.get("stage", ""))
